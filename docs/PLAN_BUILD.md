# Plan de Builds Divertidas para Pathfinder 2e Remaster

Este documento contiene 3 builds divertidas y efectivas para cada clase del Pathfinder 2e Remaster, basadas en investigación de guías populares, foros de la comunidad y recursos especializados.

---

## Índice de Clases

### Clases Marciales
- [Bárbaro](#bárbaro)
- [Campeón](#campeón)
- [Guerrero (Fighter)](#guerrero-fighter)
- [Espadachín (Swashbuckler)](#espadachín-swashbuckler)
- [Guardián](#guardián)

### Clases de Habilidad
- [Investigador](#investigador)
- [Pícaro (Rogue)](#pícaro-rogue)
- [Explorador (Ranger)](#explorador-ranger)
- [Monje](#monje)

### Lanzadores Arcanos
- [Mago](#mago)
- [Brujo (Witch)](#brujo-witch)
- [Hechicero (Sorcerer)](#hechicero-sorcerer)
- [Mago de Batalla (Magus)](#mago-de-batalla-magus)

### Lanzadores Divinos
- [Clérigo](#clérigo)
- [Oráculo](#oráculo)

### Lanzadores Primarios
- [Druida](#druida)
- [Kineticista](#kineticista)

### Lanzadores Ocultos
- [Bardo](#bardo)
- [Psíquico](#psíquico)

### Clases Especiales
- [Alquimista](#alquimista)
- [Inventor](#inventor)
- [Pistolero (Gunslinger)](#pistolero-gunslinger)
- [Invocador (Summoner)](#invocador-summoner)
- [Taumaturgo](#taumaturgo)

---

## Bárbaro

### Build 1: Furia del Dragón
**Concepto**: Canaliza el poder de un dragón ancestral, ganando aliento de dragón y eventualmente alas.
- **Instinto**: Dragón (Plata o Blanco recomendado para daño de frío - menos resistido)
- **Atributos**: FUE 18, CON 16, DES 12, SAB 12
- **Arma**: Espada bastarda o Hacha de guerra
- **Dotes clave**:
  - Nivel 1: Momento de Claridad
  - Nivel 4: Carga del Dragón
  - Nivel 6: Aliento de Furia del Dragón
  - Nivel 12: Alas de Furia del Dragón
  - Nivel 16: Transformación del Dragón
- **Estilo de juego**: Daño de energía que ignora resistencias físicas, culminando en transformación dracónica.

### Build 2: Gigante con Alcance
**Concepto**: Un bárbaro que empuña armas enormes y controla el campo de batalla.
- **Instinto**: Gigante
- **Atributos**: FUE 18, CON 16, DES 12, SAB 10
- **Arma**: Arma de tamaño Grande (gratis con el instinto) - Guadaña o Martillo de guerra
- **Dotes clave**:
  - Nivel 1: Ataque de Oportunidad (esencial)
  - Nivel 6: Estatura de Gigante (+5 pies de alcance)
  - Nivel 8: Abusón Furioso
  - Nivel 12: Estatura de Titán
  - Nivel 14: Acometida del Gigante
- **Estilo de juego**: Control del campo de batalla con alcance masivo, amenazando un área enorme.

### Build 3: Bárbaro Espiritual Totémico
**Concepto**: Conectado con espíritus animales, versátil y resistente.
- **Instinto**: Animal (Lobo, Oso o Ciervo)
- **Atributos**: FUE 16, CON 16, DES 14, SAB 12
- **Arma**: Depende del animal - Garras (Lobo), Mandíbulas (Oso)
- **Dotes clave**:
  - Nivel 1: Piel Animal
  - Nivel 4: Atleta Furioso
  - Nivel 6: Furia Renovada
  - Nivel 10: Furia Compartida (con compañero animal vía arquetipo)
- **Estilo de juego**: Más defensivo que otros bárbaros, con transformaciones animales.

---

## Campeón

### Build 1: Redentor Protector
**Concepto**: El tanque definitivo que protege aliados y debilita enemigos.
- **Causa**: Redentor
- **Atributos**: FUE 16, CON 14, CAR 16, SAB 12
- **Arma**: Espada larga + Escudo
- **Armadura**: Placas completas
- **Aliado Divino**: Escudo
- **Dotes clave**:
  - Nivel 1: Deidad con Sanación
  - Nivel 2: Bloqueo con Escudo divino
  - Nivel 4: Golpe de la Misericordia
  - Nivel 6: Escudo del Aliado
  - Nivel 8: Armadura Divina
- **Estilo de juego**: Reacción para reducir daño a aliados y Enfeeble al atacante. Muy difícil de matar.

### Build 2: Liberador de Alta Movilidad
**Concepto**: Permite a los aliados reposicionarse mientras los protege.
- **Causa**: Liberador
- **Atributos**: FUE 16, DES 14, CON 14, CAR 14
- **Arma**: Lucero del alba + Escudo
- **Aliado Divino**: Corcel (Compañero Animal)
- **Dotes clave**:
  - Nivel 1: Imposición de Manos
  - Nivel 4: Aura de Valentía
  - Nivel 6: Montura de Campeón
  - Nivel 10: Litania contra el Mal
- **Estilo de juego**: Combate montado, liberando aliados de restricciones mientras cabalga al rescate.

### Build 3: Paladín Castigador
**Concepto**: El campeón más ofensivo, con golpe de represalia.
- **Causa**: Paladín
- **Atributos**: FUE 18, CON 14, CAR 14
- **Arma**: Espada de dos manos (Espadón) o Martillo de guerra
- **Aliado Divino**: Hoja
- **Dotes clave**:
  - Nivel 1: Canalizar Castigo
  - Nivel 4: Juramento contra el Mal
  - Nivel 6: Ataque de Represalia mejorado
  - Nivel 8: Armadura Sagrada
- **Estilo de juego**: Daño extra contra el mal, contraataques frecuentes.

---

## Guerrero (Fighter)

### Build 1: Duelista de Doble Arma
**Concepto**: Dos armas, muchos ataques, alto DPS sostenido.
- **Atributos**: DES 18, CON 14, FUE 12
- **Armas**: Rapier + Espada corta o Espada Aldori + Daga
- **Armadura**: Cuero tachonado o Cota de malla ligera
- **Dotes clave**:
  - Nivel 1: Doble Corte
  - Nivel 2: Parada Gemela
  - Nivel 4: Finta Gemela
  - Nivel 6: Riposte Gemela
  - Nivel 8: Ráfaga de Doble Arma (Flourish/Press)
- **Estilo de juego**: Muchos ataques con penalización reducida, defensa con parada gemela.

### Build 2: Guerrero de Escudo Reactivo
**Concepto**: El tanque guerrero definitivo con contraataques.
- **Atributos**: FUE 18, CON 16, DES 12
- **Armas**: Espada larga/Hacha de batalla + Escudo pesado
- **Armadura**: Placas completas
- **Dotes clave**:
  - Nivel 1: Ataque Reactivo
  - Nivel 2: Bloqueo con Escudo Agresivo
  - Nivel 4: Puñetazo de Escudo
  - Nivel 6: Fortificación del Escudo
  - Nivel 8: Reflejos del Escudo
- **Estilo de juego**: Bloquea, contraataca, nunca muere. Simple pero devastador.

### Build 3: Arquero de Precisión
**Concepto**: Disparos devastadores a distancia con críticos frecuentes.
- **Atributos**: DES 18, FUE 14, SAB 12
- **Arma**: Arco largo compuesto
- **Armadura**: Cuero
- **Dotes clave**:
  - Nivel 1: Disparo en Movimiento
  - Nivel 2: Disparo Certero
  - Nivel 4: Triple Disparo
  - Nivel 6: Puntería Firme
  - Nivel 8: Disparo Penetrante
  - Nivel 10: Disparo Múltiple
- **Estilo de juego**: Alto daño a distancia, excelente precisión gracias a la progresión de competencia del guerrero.

---

## Espadachín (Swashbuckler)

### Build 1: Pistolero Fanfarrón
**Concepto**: Combina pistola y espada al estilo pirata.
- **Estilo**: Pistolero (Bravado)
- **Atributos**: DES 18, CAR 16, CON 12
- **Armas**: Pistola + Rapier
- **Dotes clave**:
  - Nivel 1: Fanfarronear
  - Nivel 2: Recarga Oportuna
  - Nivel 4: Estocada del Espadachín
  - Nivel 6: Disparo de Precisión
  - Nivel 8: Estilo del Mosquetero
- **Estilo de juego**: Dispara, gana Aplomo, acércate para el Finisher devastador.

### Build 2: Duelista Gimnástico
**Concepto**: Acrobático y elegante, usando el entorno.
- **Estilo**: Gimnasta
- **Atributos**: DES 18, CAR 14, FUE 12
- **Arma**: Rapier o Sable
- **Dotes clave**:
  - Nivel 1: Caída de Gato
  - Nivel 2: Tumbada Acrobática
  - Nivel 4: Salto del Duende
  - Nivel 6: Salto Derribador
  - Nivel 8: Evasión Elegante
- **Estilo de juego**: Volteretas, saltos, tropiezos - el espadachín más móvil.

### Build 3: Espadachín Ingenioso (Wit)
**Concepto**: Usa el ingenio y las burlas para confundir enemigos.
- **Estilo**: Ingenioso (Wit)
- **Atributos**: CAR 18, DES 16, INT 12
- **Arma**: Espada corta o Estoque
- **Dotes clave**:
  - Nivel 1: Insulto Desestabilizador
  - Nivel 2: Burla Provocadora
  - Nivel 4: Golpe Oportunista
  - Nivel 6: Charme Encantador
  - Nivel 8: Distracción Verbal
- **Estilo de juego**: Control social y mental, confunde enemigos, Face del grupo.

---

## Pícaro (Rogue)

### Build 1: Ladrón Clásico (Thief)
**Concepto**: El pícaro ágil de dagas que ataca desde las sombras.
- **Racket**: Ladrón (Thief)
- **Atributos**: DES 18, INT 14, CAR 12
- **Armas**: Dagas (arrojadizas también), Espada corta
- **Dotes clave**:
  - Nivel 1: Movilidad
  - Nivel 2: Desaparición Rápida
  - Nivel 4: Movimiento Furtivo
  - Nivel 6: Golpe Taimado
  - Nivel 8: Desviación Oportunista
- **Estilo de juego**: Entra, apuñala, desaparece. El pícaro clásico.

### Build 2: Matón Intimidante (Ruffian)
**Concepto**: El pícaro de fuerza bruta que intimida.
- **Racket**: Rufián (Ruffian)
- **Atributos**: FUE 18, CON 14, DES 14
- **Armas**: Maza, Sap, o armas d8
- **Armadura**: Armadura media
- **Dotes clave**:
  - Nivel 1: Intimidación Brutal
  - Nivel 2: Nocaut
  - Nivel 4: Golpe Intimidante
  - Nivel 6: Postura Amenazante
  - Nivel 8: Mirada Aterradora
- **Estilo de juego**: Golpea fuerte, aterroriza, deja inconscientes. Menos sigiloso, más contundente.

### Build 3: Cerebro Criminal (Mastermind)
**Concepto**: El pícaro que usa conocimiento para encontrar debilidades.
- **Racket**: Cerebro (Mastermind)
- **Atributos**: INT 18, DES 16, CAR 12
- **Armas**: Ballesta de mano, Rapier
- **Dotes clave**:
  - Nivel 1: Recordar Conocimiento en combate
  - Nivel 2: Evaluación Rápida
  - Nivel 4: Trucos del Oficio (multiclase Investigador recomendado)
  - Nivel 6: Explotación de Debilidad
  - Nivel 8: Conocimiento Estratégico
- **Estilo de juego**: Identifica enemigos, comunica debilidades al grupo, ataca puntos vitales.

---

## Explorador (Ranger)

### Build 1: Explorador de Ráfaga con Compañero
**Concepto**: Muchos ataques con tu compañero animal.
- **Hunter's Edge**: Ráfaga (Flurry)
- **Atributos**: DES 18, SAB 14, CON 12
- **Armas**: Dos espadas cortas o Hacha de mano + Hacha arrojadiza
- **Compañero Animal**: Lobo (Derribar) o Leopardo (Velocidad)
- **Dotes clave**:
  - Nivel 1: Compañero Animal
  - Nivel 2: Derribo Doble
  - Nivel 4: Compañero Maduro
  - Nivel 6: Compañero Increíble
  - Nivel 8: Grito del Compañero
- **Estilo de juego**: 4 ataques propios + 2-3 del compañero por turno. Abrumador.

### Build 2: Arquero de Precisión
**Concepto**: Un disparo devastador por turno.
- **Hunter's Edge**: Precisión
- **Atributos**: DES 18, SAB 14, FUE 12
- **Arma**: Arco largo compuesto
- **Dotes clave**:
  - Nivel 1: Disparo Cazador
  - Nivel 2: Apuntar
  - Nivel 4: Tiro Corriendo
  - Nivel 6: Disparo Penetrante
  - Nivel 8: Doble Presa
- **Estilo de juego**: Menos ataques pero más daño por disparo. Ideal para ballestas también.

### Build 3: Explorador de Artimañas (Outwit)
**Concepto**: El explorador de habilidades, experto en rastreo y engaño.
- **Hunter's Edge**: Artimañas (Outwit)
- **Atributos**: SAB 16, DES 16, CAR 14
- **Arma**: Arco corto + Espada corta
- **Dotes clave**:
  - Nivel 1: Conocimiento del Monstruo
  - Nivel 2: Rastreo Rápido
  - Nivel 4: Camuflaje del Terreno
  - Nivel 6: Escondite Propio
  - Nivel 8: Cazador Evasivo
- **Estilo de juego**: El scout definitivo. Bonus a Sigilo, Engaño, Intimidación contra la presa.

---

## Monje

### Build 1: Monje de la Grulla (Tanque)
**Concepto**: El monje más defensivo, difícil de golpear.
- **Stance**: Postura de la Grulla
- **Atributos**: DES 18, SAB 16, CON 14
- **Dotes clave**:
  - Nivel 1: Postura de la Grulla (+1 CA, ataques de ala)
  - Nivel 2: Flurry de Golpes
  - Nivel 4: Posición Defensiva
  - Nivel 6: Aleteo de Grulla
  - Nivel 8: Bloqueo con Palma
- **Estilo de juego**: CA altísima, aguanta en primera línea, contraataques elegantes.

### Build 2: Monje Dragón (Daño)
**Concepto**: El monje más ofensivo con aliento de dragón.
- **Stance**: Postura del Dragón
- **Atributos**: FUE 18, SAB 14, CON 14
- **Dotes clave**:
  - Nivel 1: Postura del Dragón (d10 daño)
  - Nivel 2: Rugido del Dragón
  - Nivel 4: Aliento del Dragón
  - Nivel 6: Garra del Dragón
  - Nivel 8: Embestida del Dragón
- **Estilo de juego**: Alto daño cuerpo a cuerpo más daño de área ocasional.

### Build 3: Monje Ki Místico
**Concepto**: Combina artes marciales con poderes sobrenaturales.
- **Stance**: Flexible (Wolf Stance o Mountain Stance)
- **Atributos**: SAB 18, DES 16, CON 14
- **Dotes clave**:
  - Nivel 1: Golpe Ki
  - Nivel 2: Ráfaga Ki
  - Nivel 4: Carrera Ki
  - Nivel 6: Forma del Viento
  - Nivel 8: Puñetazo Elemental
  - Nivel 12: Vuelo Abundante
- **Estilo de juego**: Poderes sobrenaturales, movilidad mágica, versatilidad.

---

## Mago

### Build 1: Evocador de Batalla
**Concepto**: Especialista en magia de daño de área.
- **Escuela**: Escuela de la Destrucción (o Sin escuela para flexibilidad)
- **Atributos**: INT 18, CON 14, DES 12
- **Conjuros clave**:
  - Trucos: Rayo de Escarcha, Cuchillas Eléctricas
  - Nivel 1: Manos Ardientes, Proyectil Mágico
  - Nivel 2: Bola de Fuego Menor, Flecha Ácida
  - Nivel 3: Bola de Fuego, Relámpago
- **Dotes clave**:
  - Nivel 1: Conjuro Alcanzado
  - Nivel 2: Canalizar Elementos
  - Nivel 4: Conjuro Ensanchado
- **Estilo de juego**: Explosiones masivas, control de multitudes con daño.

### Build 2: Mago de Control
**Concepto**: Manipula el campo de batalla sin necesidad de tiradas de salvación enemigas.
- **Escuela**: Ilusión o Conjuración
- **Atributos**: INT 18, SAB 14, CAR 12
- **Conjuros clave**:
  - Trucos: Mano Mágica, Prestidigitación
  - Nivel 1: Grasa, Niebla Oscurecedora
  - Nivel 2: Telaraña, Oscuridad
  - Nivel 3: Ralentizar, Muros elementales
- **Dotes clave**:
  - Nivel 1: Familar
  - Nivel 4: Vínculo Mejorado
  - Nivel 6: Conjuro Silencioso
- **Estilo de juego**: Crea obstáculos, dificulta el movimiento enemigo, protege al grupo.

### Build 3: Mago de Invocación
**Concepto**: Llena el campo de batalla con criaturas invocadas.
- **Escuela**: Conjuración
- **Atributos**: INT 18, CON 14, SAB 12
- **Conjuros clave**:
  - Nivel 1: Invocar Construcción
  - Nivel 2: Invocar Elemental
  - Nivel 3: Invocar Animal/Planta
  - Niveles altos: Invocar criaturas progresivamente más poderosas
- **Dotes clave**:
  - Nivel 2: Invocación Aumentada
  - Nivel 4: Magia de Vínculo
  - Nivel 8: Invocación en Cadena
- **Estilo de juego**: Economía de acción masiva, muchos cuerpos en el campo.

---

## Hechicero (Sorcerer)

### Build 1: Hechicero Dracónico (Blaster)
**Concepto**: El blaster definitivo con sangre de dragón.
- **Linaje**: Dracónico (Plata o Blanco - frío)
- **Tradición**: Arcano
- **Atributos**: CAR 18, CON 14, DES 12
- **Conjuros clave**:
  - Trucos: Rayo de Escarcha
  - Nivel 3: Aliento del Dragón (Focus Spell)
  - Nivel 3+: Bola de Fuego, Cono de Frío
- **Dotes clave**:
  - Nivel 1: Peligroso Hechicería
  - Nivel 4: Metamagia de Linaje
  - Nivel 6: Invocación del Linaje
- **Estilo de juego**: Daño masivo de área, resistencia al tipo de daño elegido, escamas de dragón.

### Build 2: Hechicero Angélico (Support)
**Concepto**: Sanador y protector con sangre celestial.
- **Linaje**: Angélico
- **Tradición**: Divino
- **Atributos**: CAR 18, SAB 14, CON 14
- **Conjuros clave**:
  - Sanación, Bendición, Restauración
  - Focus: Aureola de Sanación
- **Dotes clave**:
  - Nivel 1: Sangre de los Cielos
  - Nivel 4: Sanación Divina
  - Nivel 8: Aura de Protección
- **Estilo de juego**: El sanador espontáneo, mejor que el clérigo en situaciones imprevistas.

### Build 3: Hechicero Aberrante (Control)
**Concepto**: Magia alienígena y tentáculos.
- **Linaje**: Aberrante
- **Tradición**: Oculto
- **Atributos**: CAR 18, CON 14, INT 12
- **Conjuros clave**:
  - Conjuros de toque con alcance extendido
  - Control mental, confusión
  - Focus: Miembros Tentaculares
- **Dotes clave**:
  - Nivel 1: Conjuro Alcanzado (esencial)
  - Nivel 4: Anatomía Aberrante
  - Nivel 8: Forma Alienígena
- **Estilo de juego**: Raro y perturbador, extiende toque a 10 pies, control mental.

---

## Clérigo

### Build 1: Clérigo de Batalla (Warpriest)
**Concepto**: En primera línea con armadura pesada y arma.
- **Doctrina**: Sacerdote de Guerra (Warpriest)
- **Deidad**: Gorum (Espadón), Iomedae (Espada larga), o Sarenrae (Cimitarra)
- **Atributos**: SAB 16, FUE 16, CON 14
- **Dominio**: Guerra, Celo, o Confianza
- **Dotes clave**:
  - Nivel 1: Arma Letal (Deadly Simplicity)
  - Nivel 2: Armadura de Fe
  - Nivel 4: Canalizar Castigo
  - Nivel 6: Escudo de Fe
- **Estilo de juego**: Tanque que sana, combate cuerpo a cuerpo con magia de apoyo.

### Build 2: Clérigo Sanador (Cloistered Cleric)
**Concepto**: El mejor sanador del juego.
- **Doctrina**: Clérigo Enclaustrado
- **Deidad**: Sarenrae, Farasma, o Shelyn
- **Atributos**: SAB 18, CAR 14, CON 14
- **Font**: Fuente de Sanación
- **Dotes clave**:
  - Nivel 1: Sanación de Comunidad
  - Nivel 2: Canalización Selectiva
  - Nivel 4: Despertar
  - Nivel 8: Sanación Masiva
- **Estilo de juego**: Sanaciones de área gratuitas, mantiene al grupo vivo sin esfuerzo.

### Build 3: Clérigo de Daño (Harm Font)
**Concepto**: El clérigo ofensivo que canaliza energía negativa.
- **Doctrina**: Sacerdote de Guerra o Enclaustrado
- **Deidad**: Urgathoa, Zon-Kuthon, o Farasma
- **Atributos**: SAB 18, CON 14, CAR 12
- **Font**: Fuente de Daño
- **Dotes clave**:
  - Nivel 1: Daño de Comunidad
  - Nivel 2: Canalización Selectiva
  - Nivel 4: Necrosis
  - Nivel 6: Toque Vampírico mejorado
- **Estilo de juego**: Daña a todos los vivos en área, excelente contra hordas.

---

## Druida

### Build 1: Druida Salvaje (Wild Shape)
**Concepto**: Se transforma en bestias poderosas.
- **Orden**: Salvaje (Wild)
- **Atributos**: SAB 18, CON 14, FUE 14
- **Dotes clave**:
  - Nivel 1: Forma Salvaje
  - Nivel 2: Forma de Insecto
  - Nivel 4: Forma de Planta
  - Nivel 6: Forma Elemental
  - Nivel 8: Forma de Dragón
  - Nivel 12: Forma Perfecta
- **Estilo de juego**: Cambia de forma según la situación, tanque temporal cuando se transforma.

### Build 2: Druida de Tormenta (Blaster)
**Concepto**: Controla el clima y lanza rayos.
- **Orden**: Tormenta
- **Atributos**: SAB 18, DEX 14, CON 12
- **Conjuros clave**:
  - Llamar Relámpago, Control del Clima
  - Tempest Surge (Focus)
- **Dotes clave**:
  - Nivel 1: Ira de la Tormenta
  - Nivel 4: Relámpago del Druida
  - Nivel 6: Vientos Protectores
- **Estilo de juego**: Daño de área eléctrico, control del campo con viento y lluvia.

### Build 3: Druida Animal (Compañero)
**Concepto**: Lucha junto a un compañero animal poderoso.
- **Orden**: Animal
- **Atributos**: SAB 16, CON 14, DES 14
- **Compañero**: Oso (tanque), Lobo (flanqueo), o Dinosaurio
- **Dotes clave**:
  - Nivel 1: Compañero Animal (orden)
  - Nivel 4: Compañero Maduro
  - Nivel 6: Compañero Increíble
  - Nivel 8: Especialización del Compañero
- **Estilo de juego**: Tu compañero es tan importante como tú, trabajan en equipo.

---

## Oráculo

### Build 1: Oráculo de las Llamas
**Concepto**: Control del fuego con maldición de calor.
- **Misterio**: Llamas
- **Atributos**: CAR 18, CON 14, SAB 12
- **Maldición**: Ardor (ganas debilidad al frío pero poderes de fuego)
- **Dotes clave**:
  - Nivel 1: Revelación del Fuego
  - Nivel 4: Forma Ardiente
  - Nivel 6: Conjuros de Fuego mejorados
- **Estilo de juego**: Blaster divino, gestiona tu maldición para maximizar daño.

### Build 2: Oráculo de Huesos (Necromante)
**Concepto**: Controla muertos vivientes y drena vida.
- **Misterio**: Huesos
- **Atributos**: CAR 18, CON 16, SAB 12
- **Dotes clave**:
  - Nivel 1: Toque de Muerte
  - Nivel 4: Invocar No-Muerto
  - Nivel 6: Armadura de Huesos
  - Nivel 8: Legión de No-Muertos
- **Estilo de juego**: Necromante divino, rodéate de servidores esqueléticos.

### Build 3: Oráculo de la Vida (Sanador)
**Concepto**: El sanador más poderoso pero con maldición de dolor.
- **Misterio**: Vida
- **Atributos**: CAR 18, SAB 14, CON 14
- **Dotes clave**:
  - Nivel 1: Enlace Vital
  - Nivel 4: Sanación de Revelación
  - Nivel 6: Canalización de Vida
- **Estilo de juego**: Sanación masiva pero tu maldición te daña. Gestiona el riesgo.

---

## Bardo

### Build 1: Bardo de Inspiración (Buff Master)
**Concepto**: El mejor buffer del juego.
- **Musa**: Maestro de Ceremonias o Guerrero
- **Atributos**: CAR 18, DEX 14, CON 12
- **Composiciones clave**:
  - Inspirar Coraje (+1 ataque y daño)
  - Inspirar Defensa (+1 CA)
  - Dirge of Doom (-1 a enemigos)
- **Dotes clave**:
  - Nivel 1: Armonía Lingering
  - Nivel 4: Inspire Heroics
  - Nivel 6: Dirección de la Batalla
- **Estilo de juego**: Mantén inspiración constantemente, el grupo entero mejora.

### Build 2: Bardo de Ocultismo (Debuffer)
**Concepto**: Debilita a los enemigos con magia mental.
- **Musa**: Enigma
- **Atributos**: CAR 18, INT 14, SAB 12
- **Conjuros clave**:
  - Miedo, Risas Histéricas, Sugestión
  - Canción de Perdición
- **Dotes clave**:
  - Nivel 1: Conocimiento Bardico
  - Nivel 4: Canto Hipnótico
  - Nivel 6: Control Mental
- **Estilo de juego**: Controla enemigos, reduce sus tiradas, Face del grupo.

### Build 3: Bardo de Combate (Valor)
**Concepto**: El bardo que pelea en primera línea.
- **Musa**: Guerrero
- **Atributos**: CAR 16, DEX 16, CON 14
- **Armas**: Rapier + Escudo o Arco
- **Dotes clave**:
  - Nivel 1: Composición de Guerra
  - Nivel 2: Competencia Marcial
  - Nivel 4: Contraataque Inspirado
  - Nivel 6: Fortissimo
- **Estilo de juego**: Pelea mientras cantas, inspira con el ejemplo.

---

## Alquimista

### Build 1: Bombardero
**Concepto**: Lanza bombas explosivas constantemente.
- **Campo de Investigación**: Bombardero
- **Atributos**: INT 18, DEX 16, CON 12
- **Items clave**: Bombas Alquímicas de todo tipo
- **Dotes clave**:
  - Nivel 1: Lanzamiento Rápido
  - Nivel 2: Bombas Persistentes
  - Nivel 4: Megabomba
  - Nivel 6: Bombardeo de Área
- **Estilo de juego**: Boom, boom, boom. Daño de área constante.

### Build 2: Mutagenista
**Concepto**: Se potencia con mutágenos para combate cuerpo a cuerpo.
- **Campo de Investigación**: Mutagenista
- **Atributos**: INT 16, FUE 16, CON 14
- **Items clave**: Mutágeno Bestial, Mutágeno de Fuerza
- **Dotes clave**:
  - Nivel 1: Mutación Extendida
  - Nivel 4: Doble Mutágeno
  - Nivel 8: Maestría Mutagénica
- **Estilo de juego**: El Sr. Hyde alquímico, transformación temporal en bestia.

### Build 3: Quirurgo/Sanador
**Concepto**: El médico del grupo con elixires de sanación.
- **Campo de Investigación**: Quirurgo
- **Atributos**: INT 18, SAB 14, DEX 12
- **Items clave**: Elixires de Vida, Antídotos
- **Dotes clave**:
  - Nivel 1: Rápida Administración
  - Nivel 2: Cirugía Eficiente
  - Nivel 4: Elixir de Vida mejorado
- **Estilo de juego**: Sanador no-mágico, siempre tiene el item correcto.

---

## Taumaturgo

### Build 1: Taumaturgo de Espejo
**Concepto**: Usa un espejo como implemento para confundir.
- **Implemento**: Espejo
- **Atributos**: CAR 18, CON 14, DEX 14
- **Arma**: Espada de una mano + implemento
- **Dotes clave**:
  - Nivel 1: Reflejos del Espejo
  - Nivel 4: Duplicado Ilusorio
  - Nivel 6: Confusión Reflectante
- **Estilo de juego**: Crea duplicados, confunde enemigos, nunca saben cuál eres tú.

### Build 2: Taumaturgo de Arma
**Concepto**: Tu arma ES tu implemento.
- **Implemento**: Arma
- **Atributos**: CAR 16, FUE 16, CON 14
- **Arma**: Espada bastarda o Martillo de guerra
- **Dotes clave**:
  - Nivel 1: Golpe del Implemento
  - Nivel 4: Arma Encantada
  - Nivel 6: Golpe Devastador
- **Estilo de juego**: Marcial puro con trucos de cazador de monstruos.

### Build 3: Taumaturgo de Libro (Conocimiento)
**Concepto**: El experto en monstruos definitivo.
- **Implemento**: Tomo
- **Atributos**: CAR 16, INT 16, SAB 12
- **Arma**: Ballesta de mano + Tomo
- **Dotes clave**:
  - Nivel 1: Conocimiento Esotérico
  - Nivel 4: Evaluar Debilidad
  - Nivel 6: Explotar Resistencia
- **Estilo de juego**: Identifica debilidades, las explota, informa al grupo.

---

## Otras Clases

### Kineticista
1. **Fuego Puro**: Blaster de fuego, quema todo
2. **Tierra Defensivo**: Tanque elemental, armadura de piedra
3. **Multi-elemento**: Versátil, combina elementos

### Psíquico
1. **Telépata de Control**: Domina mentes enemigas
2. **Telequinético de Daño**: Lanza objetos, destruye con la mente
3. **Vidente de Apoyo**: Predice el futuro, ayuda aliados

### Inventor
1. **Armadura Potenciada**: Iron Man de fantasía
2. **Arma Experimental**: Arma con modificaciones locas
3. **Constructo Compañero**: Robot/gólem mascota

### Pistolero (Gunslinger)
1. **Pistolero Duelista**: Desenfunde rápido, un disparo mortal
2. **Francotirador**: Disparos a larga distancia, oculto
3. **Escopetero**: Daño masivo a corta distancia

### Invocador (Summoner)
1. **Eidolon Dragón**: Tu compañero es un dragón
2. **Eidolon Angel/Demonio**: Celestial o infernal
3. **Eidolon Bestia**: Animal fantástico personalizado

### Investigador
1. **Detective Forense**: CSI medieval
2. **Espía Social**: Intriga y engaño
3. **Estratega de Combate**: Conoce los puntos débiles

### Mago de Batalla (Magus)
1. **Laughing Shadow**: Móvil, teletransporte
2. **Starlit Span**: Arquero mágico
3. **Inexorable Iron**: Armadura pesada, tanque arcano

### Brujo (Witch)
1. **Familiar Poderoso**: Tu familiar hace casi todo
2. **Maldiciones Persistentes**: Debuffs constantes
3. **Patrón de Noche**: Control y oscuridad

---

## Fuentes y Referencias

- [RPGBOT - Guías de Clases Remastered](https://rpgbot.net/p2/characters/classes/)
- [Archives of Nethys - Base de Datos Oficial](https://2e.aonprd.com/)
- [Pop Culture Pathfinder - Builds temáticas](https://pathfinderbuilds.com)
- [The Gamer - Guías de Builds](https://www.thegamer.com)
- [Paizo Forums - Discusiones de la Comunidad](https://paizo.com/community)
- [Dexerto - Guías de Clases](https://www.dexerto.com/gaming/)
- [CBR - Mejores Builds para Principiantes](https://www.cbr.com/pathfinder-2e-beginner-builds/)
- [The Whiteroom - Guía de Optimización del Taumaturgo](https://the-whiteroom.github.io/thaumaturge-optimisation-guide)

---

## Notas sobre el Remaster

El **Pathfinder 2e Remaster** (Player Core 1 y 2) introdujo cambios significativos:

1. **Alineamiento eliminado**: Ahora se usa Santificado/Profano y Edictos/Anatemas
2. **Causas del Campeón**: Los términos Paladín, Redentor, etc. son genéricos
3. **Escuelas del Mago**: Reorganizadas bajo nuevos nombres
4. **Tradiciones mágicas**: Se mantienen Arcana, Divina, Oculta y Primal
5. **Nuevas clases en PC2**: Guardian, algunas actualizaciones mayores

Las builds de este documento están diseñadas para funcionar con el contenido Remastered.
