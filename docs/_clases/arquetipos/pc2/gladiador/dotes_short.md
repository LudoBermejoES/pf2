---
layout: page
permalink: /clases/arquetipos/pc2/gladiador/dotes_short/
title: "Dotes de Arquetipo: Gladiador"
chapter: Clases
category: clases
archetype: Gladiador
archetype_type: other
---

El arquetipo de Gladiador te otorga acceso a nuevas habilidades y talentos especializados. Debes cumplir todos los requisitos antes de seleccionar una dote.

---

## Nivel 2

| Dote | Descripción | Rasgos | Requisitos |
|------|-------------|--------|------------|
| [Dedicación de Gladiador](/dotes/arquetipo/gladiador/dedicacion-de-gladiador/) | Sabes convertir el combate en una forma de entretenimient... | Arquetipo, Dedicación | Interpretación impresionante |

## Nivel 4

| Dote | Descripción | Rasgos | Requisitos |
|------|-------------|--------|------------|
| [Complacer a la multitud · {% include accion.html tipo="libre" %}](/dotes/arquetipo/gladiador/complacer-a-la-multitud/) | Te luces ante el público. Intenta una prueba de Interpret... | Arquetipo, Concentrar | Dedicación de Gladiador |
| [Lucha escénica](/dotes/arquetipo/gladiador/lucha-escenica/) | Contienes los golpes para dar un mejor espectáculo. No re... | Arquetipo | Dedicación de Gladiador |
| [Movimientos elegantes](/dotes/arquetipo/gladiador/movimientos-elegantes/) | Presumes tu estilo de lucha para desanimar a tus enemigos... | Arquetipo, Habilidad | Dedicación de Gladiador, experto en Interpretación |

## Nivel 6

| Dote | Descripción | Rasgos | Requisitos |
|------|-------------|--------|------------|
| [Entrenamiento de armas vistosas](/dotes/arquetipo/gladiador/entrenamiento-de-armas-vistosas/) | Dominas armas llamativas y puedes usarlas con gran efecto... | Arquetipo | Dedicación de Gladiador |

## Nivel 8

| Dote | Descripción | Rasgos | Requisitos |
|------|-------------|--------|------------|
| [Cantar el golpe](/dotes/arquetipo/gladiador/cantar-el-golpe/) | Cuando logras Complacer a la multitud, añade lo siguiente... | Arquetipo | Complacer a la multitud |

