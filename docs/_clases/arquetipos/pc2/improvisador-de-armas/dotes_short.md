---
layout: page
permalink: /clases/arquetipos/otros/improvisador-de-armas/dotes_short/
title: "Dotes de Arquetipo: Improvisador De Armas"
chapter: Clases
category: clases
archetype: Improvisador De Armas
archetype_type: other
---

El arquetipo de Improvisador De Armas te otorga acceso a nuevas habilidades y talentos especializados. Debes cumplir todos los requisitos antes de seleccionar una dote.

---

## Nivel 2

| Dote | Descripción | Rasgos | Requisitos |
|------|-------------|--------|------------|
| [Dedicación de Improvisador de armas](/dotes/arquetipo/improvisador-de-armas/dedicacion-de-improvisador-de-armas/) | No sufres la penalización normal –2 a las tiradas de ataq... | Arquetipo, Dedicación | entrenado con armas marciales |

## Nivel 4

| Dote | Descripción | Rasgos | Requisitos |
|------|-------------|--------|------------|
| [Aporreo improvisado](/dotes/arquetipo/improvisador-de-armas/aporreo-improvisado/) | Cuando realizas un Golpe con un arma improvisada que está... | Arquetipo | Dedicación de Improvisador de armas |

## Nivel 6

| Dote | Descripción | Rasgos | Requisitos |
|------|-------------|--------|------------|
| [Golpe sorpresa](/dotes/arquetipo/improvisador-de-armas/golpe-sorpresa/) | Siempre que realizas un Golpe con un arma improvisada con... | Arquetipo | Dedicación de Improvisador de armas |

## Nivel 8

| Dote | Descripción | Rasgos | Requisitos |
|------|-------------|--------|------------|
| [Crítico improvisado](/dotes/arquetipo/improvisador-de-armas/critico-improvisado/) | Puedes aplicar efectos de especialización crítica a las a... | Arquetipo | Dedicación de Improvisador de armas |
| [Golpe improvisado · {% include accion.html tipo="1" %}](/dotes/arquetipo/improvisador-de-armas/golpe-improvisado-include-accion-html-tipo-1/) | Coges rápidamente lo que tengas cerca y atacas con ello c... | Arquetipo | Dedicación de Improvisador de armas |

## Nivel 10

| Dote | Descripción | Rasgos | Requisitos |
|------|-------------|--------|------------|
| [Golpe destrozador · {% include accion.html tipo="libre" %}](/dotes/arquetipo/improvisador-de-armas/golpe-destrozador-include-accion-html-tipo-libre/) | El arma improvisada se hace añicos, destruyéndola por com... | Arquetipo | Aporreo improvisado |

