---
layout: page
permalink: /clases/arquetipos/otros/arqueologo/
title: Arqueólogo
chapter: Clases
category: clases
source: PC2
nav_order: 301
archetype_type: other
---

## Arqueólogo

Los aventureros saquean tumbas por ganancia material, pero los verdaderos arqueólogos atesoran el conocimiento obtenido de tales lugares. Puedes lograr tus objetivos mediante el aprendizaje académico, entrenándote para superar las artimañas y trampas establecidas por pueblos antiguos y rivales, con formación mágica, o incluso con un poco de suerte inexplicable.

**Dotes adicionales:** 4.º Buscador de trampas; 10.º Retrasar trampa

### Dedicación de Arqueólogo · Dote 2

<div class="feat-traits-header" markdown="0"><span class="feat-trait">Arquetipo</span><span class="feat-trait">Dedicación</span></div>

**Prerrequisitos:** entrenado en Sociedad y Latrocinio

Eres un estudioso de los pueblos y sus historias, y estás en constante búsqueda de conocimientos y artefactos del pasado. Te conviertes en experto en Sociedad y Latrocinio, y obtienes un bonificador circunstancial +1 para Recordar conocimiento sobre historia, pueblos y culturas antiguas.

### Estudios mágicos · Dote 4

<div class="feat-traits-header" markdown="0"><span class="feat-trait">Arquetipo</span></div>

**Prerrequisitos:** Dedicación de Arqueólogo

Si bien el estudio es clave para tu éxito, un poco de magia puede ayudar a descubrir los secretos del pasado. Puedes lanzar *detectar magia*, *guía* y *leer aura* como conjuros innatos ocultos de nivel 0.

### Estudios de asentamientos · Dote 4

<div class="feat-traits-header" markdown="0"><span class="feat-trait">Arquetipo</span><span class="feat-trait">Habilidad</span></div>

**Prerrequisitos:** Dedicación de Arqueólogo

Tus estudios abren nuevos horizontes. Elige un asentamiento específico. Obtienes la dote de habilidad Saber adicional en el Saber de ese asentamiento y aprendes un único idioma común o poco común de tu elección que sea prevalente en ese asentamiento.

**Especial:** Puedes tomar esta dote múltiples veces. Cada vez que la tomes, elige un asentamiento diferente.

### Identificación académica · Dote 7

<div class="feat-traits-header" markdown="0"><span class="feat-trait">Arquetipo</span><span class="feat-trait">Habilidad</span></div>

**Prerrequisitos:** Dedicación de Arqueólogo, maestro en Sociedad

Tienes el conocimiento necesario para comprender textos antiguos y artefactos culturales. Puedes usar Sociedad al Descifrar escritura, sin importar el tipo de escritura que examines. También puedes usar Sociedad para Identificar magia cuando examinas un objeto mágico o un lugar con significado cultural.

### Suerte del arqueólogo · {% include accion.html tipo="libre" %} · Dote 8

<div class="feat-traits-header" markdown="0"><span class="feat-trait">Arquetipo</span><span class="feat-trait">Fortuna</span></div>

**Prerrequisitos:** Dedicación de Arqueólogo

**Frecuencia:** una vez por hora

**Desencadenante:** Fallas una prueba contra una trampa, como una prueba de Latrocinio para Desactivarla o una tirada de Reflejos para evitar sus efectos.

Eres más que habilidoso; tu impulso por encontrar los secretos del pasado se manifiesta como un tipo extraño de suerte. Repite la prueba fallida y usa el nuevo resultado.

### Estudios mágicos mayores · Dote 10

<div class="feat-traits-header" markdown="0"><span class="feat-trait">Arquetipo</span></div>

**Prerrequisitos:** Estudios mágicos

Amplías el alcance de tu comprensión mágica, permitiéndote encontrar el camino correcto, detectar objetos ocultos y ocultarlos de rivales inescrupulosos. Puedes lanzar *augurio*, *localizar* y *velo de privacidad* como conjuros innatos ocultos, cada uno una vez al día. Puedes lanzar *velo de privacidad* solo sobre un objeto, y se intensifica automáticamente a la mitad de tu nivel redondeado hacia arriba.

---
