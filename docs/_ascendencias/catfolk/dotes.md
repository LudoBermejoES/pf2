---
layout: page
permalink: /ascendencias/catfolk/dotes/
title: Dotes de ascendencia para Catfolk
chapter: Ascendencias
category: ascendencias
source: PC2
nav_order: 102
ancestry: Catfolk
---

A nivel 1, obtienes una dote de ascendencia, y obtienes una dote de ascendencia adicional cada 4 niveles a partir de entonces (a niveles 5, 9, 13 y 17). Como catfolk, puedes seleccionar entre las siguientes dotes de ascendencia.

---

## Nivel 1

### Siesta felina · Dote 1

<div class="feat-traits-header" markdown="0"><span class="feat-trait">Catfolk</span><span class="feat-trait">Concentración</span><span class="feat-trait">Exploración</span></div>

**Frecuencia:** una vez por hora

Puedes recuperar tu energía rápidamente con una breve siesta. Duermes durante 10 minutos, tras lo cual obtienes Puntos de Golpe temporales iguales a tu nivel que duran 1 hora.

---

### Suerte felina · Dote 1 · {% include accion.html tipo="libre" %}

<div class="feat-traits-header" markdown="0"><span class="feat-trait">Catfolk</span><span class="feat-trait">Fortuna</span></div>

**Frecuencia:** una vez por día

**Disparador:** Fallas una tirada de salvación de Reflejos.

Te apartas instintivamente del peligro. Vuelve a tirar la salvación detonante y usa el mejor resultado.

---

### Danza catfolk · Dote 1 · {% include accion.html tipo="1" %}

<div class="feat-traits-header" markdown="0"><span class="feat-trait">Catfolk</span></div>

Tienes la costumbre de estar siempre en medio cuando otras criaturas intentan moverse. Intenta una prueba de Acrobacias contra la CD de Reflejos de una criatura adyacente.

**Éxito crítico** La criatura objetivo obtiene un penalizador circunstancial –2 a las tiradas de Reflejos y queda desprevenida hasta el inicio de tu siguiente turno.

**Éxito** La criatura objetivo obtiene un penalizador circunstancial –2 a las tiradas de Reflejos hasta el inicio de tu siguiente turno.

---

### Saber catfolk · Dote 1

<div class="feat-traits-header" markdown="0"><span class="feat-trait">Catfolk</span></div>

Criarte entre catfolk te ha enseñado los valores tradicionales de libertad, custodia y reacción rápida ante problemas. Obtienes competencia entrenada en Acrobacias y Supervivencia. Si fueras a volverte entrenado automáticamente en una de esas habilidades (por tu bagaje o clase, por ejemplo), en su lugar te vuelves entrenado en una habilidad a tu elección.

También obtienes la dote general Saber adicional para Saber catfolk.

---

### Familiaridad con armas catfolk · Dote 1

<div class="feat-traits-header" markdown="0"><span class="feat-trait">Catfolk</span></div>

Obtienes acceso a todas las armas poco comunes con el rasgo catfolk. Tienes familiaridad con las armas con el rasgo catfolk además de la kama, kukri, cimitarra y hoz—para efectos de competencia, tratas cualquiera de estas que sean armas marciales como armas simples y cualquiera que sean armas avanzadas como armas marciales.

A nivel 5, siempre que obtienes un golpe crítico con una de estas armas, obtienes su efecto de especialización crítica.

---

### Colmillos de sable · Dote 1

<div class="feat-traits-header" markdown="0"><span class="feat-trait">Catfolk</span></div>

Tienes colmillos largos, naturales o aumentados. Obtienes un ataque desarmado de mandíbulas que inflige 1d6 de daño perforante. Tus mandíbulas pertenecen al grupo de pelea y tienen el rasgo desarmado.

---

### Viajero afable · Dote 1

<div class="feat-traits-header" markdown="0"><span class="feat-trait">Catfolk</span></div>

Adoptas de forma natural un talante agradable y afable al conocer a otros. Estás entrenado en Diplomacia. Si fueras a volverte entrenado automáticamente en Diplomacia (por tu bagaje o clase, por ejemplo), en su lugar te vuelves entrenado en una habilidad a tu elección.

Además, obtienes la dote de habilidad Conocedor.

---

## Nivel 5

### Garras trepadoras · Dote 5

<div class="feat-traits-header" markdown="0"><span class="feat-trait">Catfolk</span></div>

Puedes extender tus garras para ayudarte a trepar. Obtienes una Velocidad de trepar de 10 pies.

---

### Guía elegante · Dote 5

<div class="feat-traits-header" markdown="0"><span class="feat-trait">Catfolk</span></div>

Puedes mostrar cómo girar rápidamente fuera del alcance de efectos peligrosos. Puedes usar la reacción Ayudar para otorgar un bono a la tirada de Reflejos de un aliado. Como es habitual con Ayudar, necesitas prepararte usando una acción en tu turno para animar al aliado.

---

### Patas ligeras · Dote 5 · {% include accion.html tipo="2" %}

<div class="feat-traits-header" markdown="0"><span class="feat-trait">Catfolk</span></div>

Puedes equilibrarte sobre tus puntas para pisar con cuidado sobre obstrucciones. Avanzas y luego Das un paso, o Das un paso y luego Avanzas, ignorando el terreno difícil durante este movimiento.

---

### Golpe de suerte · Dote 5

<div class="feat-traits-header" markdown="0"><span class="feat-trait">Catfolk</span></div>

**Requisitos:** Suerte felina

Eres aún más afortunado que la mayoría de los catfolk. Puedes activar Suerte felina cuando fallas o fallas críticamente una tirada de salvación de Fortaleza, una tirada de salvación de Voluntad, una prueba de Acrobacias o una prueba de Atletismo además de las tiradas de Reflejos. Aun así, solo puedes usar Suerte felina una vez por día.

---

### Cazador de manada · Dote 5

<div class="feat-traits-header" markdown="0"><span class="feat-trait">Catfolk</span></div>

Evitas la atención de criaturas centradas en tus aliados. Puedes usar cobertura menor de tus aliados para Ocultarte.

---

### Saltador impulsivo · Dote 5

<div class="feat-traits-header" markdown="0"><span class="feat-trait">Catfolk</span></div>

**Requisitos:** experto en Atletismo

Tus poderosas piernas te permiten dar saltos súbitos y dramáticos. Puedes Saltar como actividad de 2 acciones para duplicar la distancia que puedes Saltar verticalmente, o Saltar como actividad de 3 acciones para triplicar la distancia que puedes Saltar verticalmente. No fallas automáticamente Saltos largos por saltar en una dirección distinta a tu Avance.

---

### Bien acicalado · Dote 5

<div class="feat-traits-header" markdown="0"><span class="feat-trait">Catfolk</span></div>

Eres meticuloso con mantenerte limpio. Obtienes un bonificador circunstancial +2 a las tiradas de salvación contra enfermedades. Si tiras un éxito en una salvación contra una enfermedad, obtienes un éxito crítico en su lugar. Si tienes una habilidad distinta que mejoraría la salvación de este modo (como la característica de clase luchador endurecido por la batalla), si obtienes un fallo crítico en la salvación, obtienes un fallo en su lugar.

---

## Nivel 9

### Arañazo agravante · Dote 9

<div class="feat-traits-header" markdown="0"><span class="feat-trait">Catfolk</span><span class="feat-trait">Enfermedad</span></div>

**Requisitos:** ataque desarmado de garra

Tus garras portan un irritante inofensivo para ti pero dañino para otros. Tus Golpes con garra infligen 1d4 de daño de veneno persistente adicional con un golpe crítico.

---

### Evadir perdición · Dote 9

<div class="feat-traits-header" markdown="0"><span class="feat-trait">Catfolk</span></div>

Puedes eludir el desastre sobrenatural. Cuando fueras a ganar la condición condenado, puedes intentar una prueba plana CD 17. Con un éxito, no ganas la condición condenado.

---

### Suerte de la camada · Dote 9

<div class="feat-traits-header" markdown="0"><span class="feat-trait">Catfolk</span></div>

**Requisitos:** Suerte felina

Cuando usas Suerte felina para repetir tirada, cualquier criatura que elijas a 10 pies que también haya fallado contra el mismo efecto (como un único conjuro) también puede repetir su tirada de salvación y usar el mejor resultado.

---

### Gruñido depredador · Dote 9 · {% include accion.html tipo="libre" %}

<div class="feat-traits-header" markdown="0"><span class="feat-trait">Catfolk</span></div>

**Requisitos:** experto en Intimidación

**Disparador:** Buscas con éxito una criatura oculta o no detectada.

Das un gruñido profundo para intentar Desmoralizar a una criatura que acabas de encontrar. No recibes el penalizador –4 por no compartir un idioma con la criatura.

---

### Paso silencioso · Dote 9 · {% include accion.html tipo="1" %}

<div class="feat-traits-header" markdown="0"><span class="feat-trait">Catfolk</span><span class="feat-trait">Flourish</span></div>

Das un paso, luego te Ocultas o te Escabulles. Aún necesitas cumplir los requisitos para Ocultarte o Escabullirte, como es normal.

---

### Escurridizo precavido · Dote 9

<div class="feat-traits-header" markdown="0"><span class="feat-trait">Catfolk</span></div>

Puedes realizar la actividad de exploración Explorar al mismo tiempo que la actividad de exploración Evitar ser notado.

---

## Nivel 13

### Maldición del gato negro · Dote 13 · {% include accion.html tipo="libre" %}

<div class="feat-traits-header" markdown="0"><span class="feat-trait">Catfolk</span><span class="feat-trait">Desgracia</span><span class="feat-trait">Ocultista</span></div>

**Frecuencia:** una vez por día

**Disparador:** Una criatura a 30 pies que puedas ver fuese a superar una salvación.

Siseas una maldición rencorosa. El objetivo repite la tirada de salvación detonante y usa el peor resultado.

---

### Aullido felino · Dote 13 · {% include accion.html tipo="libre" %}

<div class="feat-traits-header" markdown="0"><span class="feat-trait">Auditivo</span><span class="feat-trait">Catfolk</span><span class="feat-trait">Concentración</span><span class="feat-trait">Emoción</span><span class="feat-trait">Mental</span></div>

**Frecuencia:** una vez por día

**Disparador:** Un aliado a 30 pies fuese a quedar reducido a 0 PG pero no morir inmediatamente.

Das un aullido audaz, llamando a tu compañero de vuelta del borde de la inconsciencia. Tu aliado no queda inconsciente y permanece a 1 Punto de Golpe. Su condición herido aún aumenta en 1 como si hubiera estado moribundo y se hubiera recuperado.

---

## Nivel 17

### Eludir problemas · Dote 17 · {% include accion.html tipo="libre" %}

<div class="feat-traits-header" markdown="0"><span class="feat-trait">Catfolk</span></div>

**Disparador:** Una criatura te falla con un ataque cuerpo a cuerpo.

Avanza hasta tu Velocidad. Este movimiento no provoca reacciones de la criatura que te falló.

---

### Suerte fiable · Dote 17

<div class="feat-traits-header" markdown="0"><span class="feat-trait">Catfolk</span></div>

**Requisitos:** Suerte felina

Sobrevives al peligro con sorprendente regularidad. Puedes usar Suerte felina una vez por hora, en lugar de una vez por día.

---

### Diez vidas · Dote 17

<div class="feat-traits-header" markdown="0"><span class="feat-trait">Catfolk</span></div>

**Requisitos:** Evadir perdición

Cuando tu personaje fuera a morir, puedes intentar una prueba plana CD 17. Con un éxito, en su lugar quedas reducido a 0 PG y con un valor de moribundo 1 menos de lo que normalmente te mataría (típicamente moribundo 3). Esto no cambia el resultado para ninguna otra criatura afectada.
