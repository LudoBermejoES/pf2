---
layout: spell
permalink: /conjuros/miedo/
title: Miedo
chapter: Conjuros
spell_level: 1
source: PC1
---
## Miedo {% include accion.html tipo="2" %} [CONJURO 1]

<img src="{{ '/assets/images/conjuros/miedo.png' | relative_url }}" class="spell-card-image" alt="Carta de conjuro">

**Tradiciones:** arcana, divina, ocultista, primigenia
**Rango de distancia:** 30 pies (9 m); **Objetivos:** 1 criatura
**Defensa:** Voluntad; **Duracion:** variable

<div class="spell-traits-wrapper" markdown="0">
<span class="trait-tag">Concentrar</span>
<span class="trait-tag">Emocion</span>
<span class="trait-tag">Manipular</span>
<span class="trait-tag">Mental</span>
<span class="trait-tag">Miedo</span>
</div>

---

Implantas el miedo en el objetivo, que tiene que hacer una tirada de salvación de Voluntad.

**Éxito critico** El objetivo no resulta afectado.

**Éxito** El objetivo queda asustado 1.

**Fallo** El objetivo queda asustado 2.

**Fallo critico** El objetivo queda asustado 3 y huyendo durante 1 minuto.

**Potenciado (3.o)** Puedes designar como objetivo hasta a 5 criaturas.