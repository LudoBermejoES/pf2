---
layout: page
title: "Introducción"
description: "Contenido del capítulo Introducción"
---

Contenido del capítulo Introducción

## Contenido

- [Atributos]({{ '/introduccion/atributos/' | relative_url }})
- [Como se Juega]({{ '/introduccion/como-se-juega/' | relative_url }})
- [Creacion de Personajes]({{ '/introduccion/creacion-personajes/' | relative_url }})
- [Dados]({{ '/introduccion/dados/' | relative_url }})
- [Encuentros]({{ '/introduccion/encuentros/' | relative_url }})
- [Exploracion]({{ '/introduccion/exploracion/' | relative_url }})
- [Golarion y el Mar Interior]({{ '/introduccion/golarion/' | relative_url }})
- [Herramientas de Juego]({{ '/introduccion/herramientas/' | relative_url }})
- [Jugar es para Todos]({{ '/introduccion/jugar-para-todos/' | relative_url }})
- [La Primera Regla]({{ '/introduccion/primera-regla/' | relative_url }})
- [Puntos Basicos del Juego]({{ '/introduccion/puntos-basicos/' | relative_url }})
- [¿Qué es un juego de rol?]({{ '/introduccion/que-es-jdr/' | relative_url }})
- [Religion]({{ '/introduccion/religion/' | relative_url }})
- [Subir de Nivel]({{ '/introduccion/subir-nivel/' | relative_url }})
- [Terminos Clave]({{ '/introduccion/terminos-clave/' | relative_url }})
- [Tiempo Libre]({{ '/introduccion/tiempo-libre/' | relative_url }})
