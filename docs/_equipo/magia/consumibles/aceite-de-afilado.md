---
layout: page
permalink: /equipo/magia/consumibles/aceite-de-afilado/
title: Aceite de afilado
chapter: Equipo
category: equipo
source: PC2
---

**Nivel** 3; **Precio** 12 mo

<div class="feat-traits-header" markdown="0"><span class="feat-trait">Consumible</span><span class="feat-trait">Mágico</span><span class="feat-trait">Transmutación</span></div>

**Tipo:** Aceite

**Uso:** Aplicar a un arma; **Impedimenta** —

---

Este aceite plateado hace que cualquier arma cortante sea extraordinariamente afilada.

**Activar** {% include accion.html tipo="1" %} Interactuar

**Efecto** Aplicas el aceite a un arma con daño cortante. Durante 1 minuto, el arma inflige 1 punto de daño adicional con cada Golpe exitoso.

---

## Ver también

- [Consumibles mágicos](/equipo/magia/consumibles/)
