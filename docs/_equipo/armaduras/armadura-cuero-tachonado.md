---
layout: page
permalink: /equipo/armaduras/armadura-cuero-tachonado/
title: Armadura de Cuero Tachonado
chapter: Equipo
category: equipo
nav_order: 3
source: PC1
---

<img src="{{ '/assets/images/armaduras/armadura-cuero-tachonado.png' | relative_url }}" class="armor-card-image" alt="Carta de armadura">


**Precio** 3 po

---

**Bon. CA** +2; **Tope Des.** +3; **Pen. pruebas** -1; **Fuerza** +1; **Impedimenta** 1; **Grupo** Cuero; **Categoría** Armadura ligera

---

Esta armadura de cuero esta reforzada con remaches de metal y a veces pequenas placas metálicas, proporcionando gran parte de la flexibilidad de una armadura de cuero, pero con una protección mas robusta.

---

## Ver también

- [Lista de armaduras](/equipo/armaduras/)
