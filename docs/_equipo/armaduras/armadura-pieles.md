---
layout: page
permalink: /equipo/armaduras/armadura-pieles/
title: Armadura de Pieles
chapter: Equipo
category: equipo
nav_order: 4
source: PC1
---

<img src="{{ '/assets/images/armaduras/armadura-pieles.png' | relative_url }}" class="armor-card-image" alt="Carta de armadura">


**Precio** 2 po

---

**Bon. CA** +3; **Tope Des.** +2; **Pen. pruebas** -2; **Pen. Velocidad** -5 pies (1,5 m); **Fuerza** +2; **Impedimenta** 2; **Grupo** Cuero; **Categoría** Armadura intermedia

---

Una mezcla de pelaje de animal, piel gruesa y a veces cuero hervido moldeado, esta armadura proporciona protección debido a sus capas de cuero, aunque su volumen lentifica al portador y reduce su movilidad.

---

## Ver también

- [Lista de armaduras](/equipo/armaduras/)
