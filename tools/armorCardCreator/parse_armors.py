#!/usr/bin/env python3
"""
Parse armor markdown files from the PF2e wiki
Extracts armor data and trait descriptions, generates armors.json
"""
...
OUTPUT_FILE = os.path.join(SCRIPT_DIR, 'data', 'armors.json')
...
def extract_stats(content):
    """Extract armor stats from the stats line"""
    stats = {}
    ...
    for segment in segments:
        ...
        if match:
            ...
            if label == 'Bon. CA':
                ...
                stats['dex_cap'] = value
            elif label == 'Pen. pruebas':
                stats['check_penalty'] = value
...
def extract_description(content):
    """Extract armor description text"""
    ...
    if len(parts) >= 5:  # frontmatter(2) + price/traits(1) + stats(1) + description(1)
        ...
        for i, part in enumerate(parts[3:], start=3):
            ...
            if text.startswith('**Bon. CA**') or text.startswith('**Daño**'):
                continue
            # Skip empty
            if not text:
                continue
...
def parse_armor_file(filepath, trait_descriptions):
    """Parse a single armor markdown file"""
    with open(filepath, 'r', encoding='utf-8') as f:
        content = f.read()
    ...
    armor['traits'] = []
    armor['trait_descriptions'] = []

    for trait in traits:
        armor['traits'].append(trait['display'])

        # Get trait description if available
        slug = trait['slug']
        if slug in trait_descriptions:
            armor['trait_descriptions'].append({
                'name': trait['display'],
    ...
    # Determine armor weight class from category
    category = armor.get('category', '')
    if 'pesada' in category.lower():
        armor['weight_class'] = 'heavy'
    elif 'intermedia' in category.lower():
        armor['weight_class'] = 'medium'
    elif 'ligera' in category.lower():
        armor['weight_class'] = 'light'
    else:
        armor['weight_class'] = 'unarmored'

    return armor


def main():
    """Main function to parse all armors"""
    print("PF2e Armors Parser")
    print("=" * 40)
    print()
    ...
    # Find all armor files
    armor_files = glob.glob(os.path.join(ARMORS_DIR, '*.md'))

    # Filter out special files
    skip_files = ['index.md']
    armor_files = [f for f in armor_files if os.path.basename(f) not in skip_files]

    print(f"Found {len(armor_files)} armor files")
    print()

    # Parse all armors
    armors = []
    for filepath in sorted(armor_files):
        try:
            armor = parse_armor_file(filepath, trait_descriptions)
            if armor['name']:  # Only add if has a name
                armors.append(armor)
                print(f"  + {armor['name']}")
        except Exception as e:
    ...
    print()
    print(f"Parsed {len(armors)} armors")

    # Group by category
    categories = {}
    for armor in armors:
        cat = armor.get('category', 'Otros')
